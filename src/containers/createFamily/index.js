		...
		<ul>
			{ children.map((child, index) => (
				<li key={index}>
					...
					{ (child.children && child.children.length > 0) ? this.renderChildren(child.children) : null}
				</li>
			))}
		</ul>	
  )
  ...
  renderFamilyTree = () => {
    ...
    return (
      <div class="tree-root">
        <ul>
          <li>
            <a onClick={() => this.selectNode(this.state.data)}>{this.state.data.name}</a>
            <ul>
				{(children && children.length > 0) ? this.renderChildren(children) : null }
            </ul>
          </li>
        </ul>
      </div>
  ...
  render() {
    return (
      <React.Fragment>
		...
		{ Object.keys(this.state.data).length > 0 ? this.renderFamilyTree() : null}
